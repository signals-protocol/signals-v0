...
import { WAD_BI, ZERO_BI, ONE_BI } from "./constants";
...
export function handleRangeFactorApplied(event: RangeFactorAppliedEvent): void {
  ...
  for (let i = lo; i <= hi; i++) {
    ...
    if (bin == null) {
      ...
      binState.lowerTick = lowerTickBin;
      binState.upperTick = upperTickBin;
      binState.currentFactor = WAD_BI;
      binState.lastUpdated = event.block.timestamp;
      binState.updateCount = ZERO_BI;
      binState.totalVolume = ZERO_BI;
    } else {
      binState = bin;
    }
    bins.push(binState);
  ...
  for (let i = 0; i < bins.length; i++) {
    ...
    binState.lastUpdated = event.block.timestamp;
    binState.updateCount = binState.updateCount.plus(ONE_BI);
    binState.save();
  }
