...
- 2025-10-31 10:51 KST: plan.md 전체 재검토 및 Sprint R 착수 준비, branch feature/sprint-r-rounding 분기.
- 2025-10-31 11:06 KST: Market64 회귀 테스트에 손실·라운딩 검증 강화 (root sum 단조, microUSDC 편차 체크) 및 RUN_REPLAY=1 hardhat test 실행 — 요약 로그 확보.
- 2025-10-31 11:14 KST: FixedPointMathU에 네AREST 라운딩 헬퍼(fromWadNearest/Min1, wMulNearest) 추가하고 변환 스펙 강화 테스트 실행(`npx hardhat test test/unit/libraries/fixedPointMath/conversion.spec.ts`).
- 2025-10-31 11:36 KST: Core 거래 라운딩 정책 Nearest/Min1 적용 및 open/increase/decrease/close 테스트 확장, monotonic 검증 추가. 실행 로그: `npx hardhat test test/integration/trading/open.spec.ts test/integration/trading/increase.spec.ts test/integration/trading/decrease.spec.ts test/integration/trading/close.spec.ts`, `npx hardhat test test/integration/trading/claim.spec.ts`.
- 2025-10-31 11:52 KST: 최근접 라운딩 테스트를 main 기준으로 재현 — `git worktree add /tmp/signals-main main` 후 새 테스트만 반영해 `npx hardhat test test/unit/libraries/fixedPointMath/conversion.spec.ts` 실행 시 함수 미구현으로 4건 실패(TypeError). 이어 현재 브랜치에서 동일 명령 수행하여 24건 통과 확인.
- 2025-10-31 12:15 KST: LazyMulSegmentTree 최근접 라운딩 검증용 실패 테스트 추가 (`should use nearest rounding for sell-side factors`) — 기존 구현 상향 보정으로 실패 확인.
- 2025-10-31 12:48 KST: LazyMulSegmentTree.wMulNearest 적용 및 pendingFactor 결합 로직/하니스 업데이트, Underflow 시뮬레이터를 최근접 라운딩으로 조정.
- 2025-10-31 12:58 KST: 문서 업데이트 (`website/docs/.../cost-rounding.md` EN/KR) — 세그먼트 트리 라운딩 정책 전환 반영.
- 2025-10-31 13:05 KST: 시험 실행 `npx hardhat test test/unit/libraries/lazyMulSegmentTree/*.spec.ts` 재검증 (97 passing).
- 2025-10-31 13:10 KST: 통합 거래 플로우 재검증 `npx hardhat test test/integration/trading/open.spec.ts ... close.spec.ts` (41 passing).
- 2025-10-31 14:22 KST: Sprint R-4 범위 재확인, SDK 파리티/RangeFactor 보정 테스트 설계 및 `clmsr-sdk/tests/rounding.nearest.test.ts`에 `wDivUp` 케이스 추가로 TDD 실패 재현(TS2724).
- 2025-10-31 14:41 KST: `clmsr-sdk/src/utils/math.ts`에 `wDivUp` 구현 및 `CLMSRSDK` 비용/수익 계산 경로를 최근접 라운딩으로 정렬, Hardhat 파리티 스위트(`test/unit/sdk/cost-proceeds.parity.spec.ts`) 작성.
- 2025-10-31 14:47 KST: 테스트 실행 — `npm test --prefix clmsr-sdk` (29 passing), `npx hardhat test test/unit/sdk/cost-proceeds.parity.spec.ts` (1 passing)로 SDK/코어 정합성 확인.
- 2025-10-31 14:53 KST: Subgraph `computeRangeFactorCorrection`에 residual 분배 헬퍼(`adjustResidual`) 도입 및 `applyResidualForTest` 유닛 테스트 작성(±잔여 케이스).
- 2025-10-31 14:57 KST: `npm run test:citrea:dev --prefix clmsr-subgraph` 실행(33 테스트 passing)으로 RangeFactor 보정 로직 업데이트 검증.
- 2025-10-31 15:05 KST: SDK 파리티 테스트를 코어 퍼블릭 뷰 기반으로 리팩터링(`test/unit/sdk/cost-proceeds.parity.spec.ts`), `CLMSRMathHarness` 테스트 전용 노출 제거. `npm test --prefix clmsr-sdk` 및 `npx hardhat test test/unit/sdk/cost-proceeds.parity.spec.ts` 재실행 그린.
- 2025-10-31 15:12 KST: Subgraph residual 테스트가 `applyResidualForTest`에 의존하지 않도록 재작성, Matchstick 스위트 `npm run test:citrea:dev --prefix clmsr-subgraph` 재검증(33 passing).
