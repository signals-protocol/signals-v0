...
결제 토큰이 6자리 소수를 사용하기 때문에 `_maxSafeChunkQuantity`가 모든 청크를 `1e12` WAD(마이크로 USDC 한 단위)에 맞춰 정규화합니다. 코어는 청크별 견적을 마이크로 단위로 올림해 누적하므로, 동일한 거래를 순차적으로 실행했을 때와 비교해 최대 `(청크 수 − 1)` 마이크로 USDC 이내의 오차 범위에 들어오도록 유지합니다. 다양한 α·수량 조합을 대상으로 한 단위 테스트가 이 라운딩 파리티를 고정하고 있습니다.

## 편측 라운딩 정책

백서가 요구하는 바와 같이 편측(한쪽 방향) 라운딩을 적용해 편향 없이도 최소 비용 보장을 유지합니다.

| 액션 | 라운딩 방식 | 사용 함수 |
| --- | --- | --- |
| 매수 / 증가 | 올림(최소 1 μUSDC 보장) | `fromWadRoundUp` |
| 매도 / 감소 / 종료 | 내림 | `fromWad` |
| 정산 지급 | 포지션 수량(이미 6자리 소수) | — |

**구현 현황:** 현재 코어·SDK·운영 스크립트는 거래당 단 한 번만 6자리 소수로 변환하며 위 헬퍼들을 사용합니다. 각 엔드포인트는 자금을 이동하기 직전에 `_roundDebit` 또는 `_roundCredit`을 한 번 호출하므로, 향후 변경이 두 번째 변환을 추가하려면 명시적으로 코드가 늘어나야 합니다. 거래를 여러 청크로 나누면 매수 비용은 증가하거나 같고, 매도 수익은 감소하거나 같다는 사실이 새 테스트로 검증되어 있습니다.
...
> **구현 메모:** `fromWadRoundUp`은 $10^{12} - 1$을 더한 뒤 $10^{12}$로 나누기 때문에 양수 입력은 항상 최소 1 마이크로 단위로 반올림됩니다. 극단적인 WAD 값에서도 0으로 감기지 않아 무상 공격을 방지합니다.

## 라운딩 헬퍼 레퍼런스

- `fromWadRoundUp`은 매수 경로에서 최소 1 μUSDC를 보장하는 올림 변환을 수행합니다.
- `fromWad`는 매도 경로에서 내림 변환을 수행해 수익이 반올림으로 증가하지 않도록 합니다.
- `wMulNearest`는 세그먼트 트리 내부 곱셈에서 사용되는 최근접 곱셈 헬퍼입니다.
- 과거 헬퍼인 `fromWadNearest`, `fromWadNearestMin1`는 분석·호환성 유지를 위해 남아 있으나 실제 거래 경로에서는 사용하지 않습니다.

## 코드에서 확인할 위치

- `LazyMulSegmentTree.sol`이 청크 분할과 지수 가중치 업데이트를 구현합니다.
- `FixedPointMath.sol`은 `fromWadRoundUp`, `fromWad`, `wMulNearest` 등 핵심 라운딩 헬퍼를 제공하며, 과거 최근접 헬퍼는 도구용으로만 유지됩니다.
